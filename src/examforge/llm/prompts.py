...
- analysis_steps 应在 6000 字以内完整回答；接近输出上限时优先结束推导并闭合 JSON，禁止输出半截字符串或未闭合 JSON。
...
- analysis_steps: Markdown 格式详细推导步骤;必须分节换行,数学公式用标准 LaTeX,关键公式单独用 $$...$$ 展示;需要覆盖审题、转化、计算、验证、易错点/假设;若使用全网搜索参考,列出参考来源标题
