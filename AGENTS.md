...
- Framework(s): FastAPI <0.115, SQLModel >=0.0.22, Typer >=0.12, Jinja2 >=3.1
- Package manager: uv (`uv.lock`)
- Runtime / deployment target: Uvicorn; Linux systemd deployment via `deploy.sh`
...
- Lint: `TODO`
- Typecheck: `TODO`
- Run locally: `uv run examforge serve --host 0.0.0.0 --port 8000`

Prefer single-file or single-test runs during iteration. Full suites are for the final verification pass.

### Layout
- Source lives in: `src/examforge/` (application package) and `src/asgi.py` (ASGI entry point)
- Tests live in: `tests/`, with acceptance fixtures and runner in `tests/acceptance/`
...
- Testing pattern and framework: pytest; test modules mirror application areas under `tests/`
